...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, temb_dim=None, dropout=0.1, skip_rescale=True,
        ...
        if lemb_dim is not None:
            self.dense1 = nn.Linear(lemb_dim, out_ch)
    ...
    def forward(self, x, # ["batch", "in_ch", "H", "W"]
        ...
        if temb is not None:
            temb_added = self.dense0(self.act(temb))
            h += temb_added[:,:,None,None]
            if lemb is not None:
                lemb_added = self.dense1(self.act(lemb))
                # lemb_norm = lemb_added.norm()
                # temb_norm = temb_added.norm().detach()
                # if lemb_norm > temb_norm + 1e-6: # comment out here for omitting normalization
                #     lemb_added = lemb_added * (temb_norm / lemb_norm)
                h += lemb_added[:,:,None,None]
...
class UNet(nn.Module):
    def __init__(self, ch, num_res_blocks, num_scales, ch_mult, input_channels,
        ...
        do_lam_embed=False, lam_scale_factor=None, lam_embed_dim=None, lam_raw_dim=None):
        super().__init__()
        ...
        self.do_lam_embed = do_lam_embed # Whether to integrate out the product measure model
        if self.do_lam_embed:
            self.lam_scale_factor = time_scale_factor if lam_scale_factor is None else lam_scale_factor
            self.lam_embed_dim = time_embed_dim if lam_embed_dim is None else lam_embed_dim
        else:
            self.lam_scale_factor = None
            self.lam_embed_dim = None
        self.lam_raw_dim = lam_raw_dim
        ...
        if self.do_lam_embed:
            self.lemb_modules = []
            self.lemb_modules.append(nn.Linear(self.lam_embed_dim, self.lam_embed_dim*4))
            nn.init.zeros_(self.lemb_modules[-1].bias)
            # nn.init.zeros_(self.lemb_modules[-1].weight)
            self.lemb_modules.append(nn.Linear(self.lam_embed_dim*4, self.lam_embed_dim*4))
            nn.init.zeros_(self.lemb_modules[-1].bias)
            # nn.init.zeros_(self.lemb_modules[-1].weight)
            self.lemb_modules = nn.ModuleList(self.lemb_modules)

            self.hlemb_modules = []
            self.hlemb_modules.append(nn.Linear(self.lam_embed_dim, self.lam_embed_dim*4))
            nn.init.zeros_(self.hlemb_modules[-1].bias)
            # nn.init.zeros_(self.hlemb_modules[-1].weight)
            self.hlemb_modules.append(nn.Linear(self.lam_embed_dim*4, self.lam_embed_dim*4))
            nn.init.zeros_(self.hlemb_modules[-1].bias)
            # nn.init.zeros_(self.hlemb_modules[-1].weight)
            self.hlemb_modules = nn.ModuleList(self.hlemb_modules)

            

        self.expanded_time_dim = 4 * self.time_embed_dim if self.time_embed_dim is not None else None
        self.expanded_lam_dim = 4 * self.lam_embed_dim if self.lam_embed_dim is not None else None

        if self.do_lam_embed and self.do_time_embed:
            self.tlam_mix = nn.Linear(self.time_embed_dim + self.lam_embed_dim, self.lam_embed_dim)
            nn.init.zeros_(self.tlam_mix.bias)
            ...
            for res_count in range(self.num_res_blocks):
                ...
                self.downsampling_modules.append(
                    ResBlock(in_ch, out_ch, temb_dim=self.expanded_time_dim,
                        ...
                        lemb_dim=self.expanded_lam_dim)
                )
                in_ch = out_ch
                h_cs.append(in_ch)
        ...
        if self.do_lam_embed and self.do_time_embed:
            self.h_t_lam_mix = nn.Linear(self.lam_embed_dim + self.time_embed_dim + in_ch, self.lam_embed_dim)
            nn.init.zeros_(self.h_t_lam_mix.bias)
            # nn.init.zeros_(self.h_t_lam_mix.weight)
    ...
    def _time_embedding(self, timesteps, lams=None):
        if self.do_time_embed:
    ...
    def _lam_embedding(self, lams, timesteps=None, h_middle=None):
        if self.do_lam_embed:
            lemb = network_utils.transformer_timestep_embedding(
                lams.view(-1) * self.lam_scale_factor, self.lam_embed_dim
            )
            if timesteps is not None:
                if h_middle is None:
                    ...
                    lemb = torch.cat((lemb, temb), 1)
                    lemb = self.act(self.tlam_mix(lemb))
                else:
                    temb = network_utils.transformer_timestep_embedding(
                        timesteps * self.time_scale_factor, self.time_embed_dim
                    )
                    hemb = h_middle.mean(dim=3).mean(dim=2)
                    lemb = torch.cat((lemb, temb, hemb), 1)
                    lemb = self.act(self.h_t_lam_mix(lemb))
            if h_middle is None:
                lemb = self.lemb_modules[0](lemb)
                lemb = self.lemb_modules[1](self.act(lemb))
            else:
                lemb = self.hlemb_modules[0](lemb)
                lemb = self.hlemb_modules[1](self.act(lemb))
        else:
            return None
    ...
    def _do_downsampling(self, h, hs, temb, lemb):
        m_idx = 0
        for scale_count in range(self.num_scales):
            for res_count in range(self.num_res_blocks):
                h = self.downsampling_modules[m_idx](h, temb, lemb)
                m_idx += 1
                if scale_count == self.scale_count_to_put_attn:
    ...
    def _do_middle(self, h, temb, lemb):
        m_idx = 0
        h = self.middle_modules[m_idx](h, temb, lemb)
        m_idx += 1
    ...
    def _do_upsampling(self, h, hs, temb, lemb):
        m_idx = 0
        for scale_count in reversed(range(self.num_scales)):
            for res_count in range(self.num_res_blocks+1):
                h = self.upsampling_modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb, lemb)
                m_idx += 1
    ...
    def forward(self,
        ...
        lemb = self._lam_embedding(lams, timesteps)

        h, hs = self._do_input_conv(h)

        h, hs = self._do_downsampling(h, hs, temb, lemb)

        lemb = self._lam_embedding(lams, timesteps, h)

        h = self._do_middle(h, temb, lemb)

        h = self._do_upsampling(h, hs, temb, lemb)
