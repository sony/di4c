...
def one_step_sampling(model, N, D, S, eps_ratio, x_t, ts, delta, p0t=None, batch_size=1):
    device=model.device
    with torch.no_grad():
        ...
        if p0t is None:
            p0t = F.softmax(model(x_t, ts), dim=2) # (N, D, S)
        ...
        qt0_denom = qt0[
            ...
            x_t.long().flatten().repeat_interleave(S)
        ...
        x_t = x_t.expand(N*batch_size, -1)

        diffs = torch.arange(S, device=device).view(1,1,S) - x_t.view(N*batch_size,D,1)
        poisson_dist = torch.distributions.poisson.Poisson(
            reverse_rates.expand(N*batch_size, -1, -1) * delta.view(-1, 1, 1))
