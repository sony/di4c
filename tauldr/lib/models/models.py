...
class ImageX0PredBase(nn.Module):
    def __init__(self, cfg, device, rank=None):
        ...
        do_lam_embed = cfg.model.do_lam_embed
        lam_scale_factor = cfg.model.time_scale_factor
        lam_embed_dim = cfg.model.time_embed_dim
        lam_raw_dim = cfg.model.lam_raw_dim
        ...
        tmp_net = networks.UNet(
                ...
                time_embed_dim,
                do_lam_embed, lam_scale_factor, lam_embed_dim, lam_raw_dim
        ).to(device)
        if cfg.distributed:
    ...
    def forward(self,
        ...
        net_out = self.net(x, times, lams) # (B, 2*C, H, W)
...
class GaussianTargetRate():
    ...
    def transition(self, t, s=None# ["B"]
    ):
        B = t.shape[0]
        S = self.S
        ...
        integral_rate_scalars = self._integral_rate_scalar(t)
        if s is not None:
            integral_rate_scalars -= self._integral_rate_scalar(s)
